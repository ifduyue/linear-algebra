...
Sets are the most familiar kind of collection, and furthermore
in practice everyone refers to these collections as sets so 
we will do the same.
...
(ii)~repeats collapse, so that the set $\set{1,1,2}$ equals the set
$\set{1,2}$.
...
\begin{equation*}
  ...
  \end{mat}
\end{equation*}
On the left the set of matrix rows is 
$\set{\rowvec{1 &1 &1}, \rowvec{2 &2 &2}, \rowvec{1 &2 &3}}$, and is
linearly dependent.
On the right the set of rows is  
$\set{\rowvec{1 &1 &1}, \rowvec{1 &1 &1}, \rowvec{1 &2 &3}}$. 
Because repeats collapse that equals
the set
$\set{\rowvec{1 &1 &1}, \rowvec{1 &2 &3}}$, 
which is linearly independent.
Oops, Gauss's Method should preserve linear dependence.

That is, strictly speaking,  
we need a type of collection where repeats do not collapse.
Such a collection is a
...
refer to a linearly independent or dependent `set.' 
