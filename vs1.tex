...
\end{equation*}
These operations
are just the ones of \( \Re^2 \), reused on its subset $L$.
...
As for the prior condition, observe here that if $\vec{v}\in L$, so that
...
suppose that $r\in\R$ and $\vec{v}\in L$
\begin{equation*}
  \vec{v}=\colvec{x \\ y}
\end{equation*}
so that it satisfies the restriction~$y=3x$.
Then 
\begin{equation*}
  r\cdot\vec{v}=r\cdot\colvec{x \\ y}
  =\colvec{rx \\ ry}
\end{equation*}
is also a member of $L$:
the fact that its second component is three times
its first $ry=3(rx)$ follows from the restriction on~$\vec{v}$.
Next, this checks~(7).
\begin{equation*}
  (r+s)\cdot\colvec{x \\ y}
  =\colvec{(r+s)x \\ (r+s)y}
