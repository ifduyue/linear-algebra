...
\begin{equation*}
  P=\set{ \colvec{x \\ y}  \suchthat y=3x}
\end{equation*}
We shall verify that it is a
a vector space, under the usual meaning of `+' and `\(\cdot\)'.
\begin{equation*}
  \colvec{x_1 \\ y_1}
  +
  \colvec{x_2 \\ y_2}
...
\end{equation*}
These operations
are just the ones of \( \Re^2 \), reused on its subset $P$.
...
suppose that we start with two vectors 
\begin{equation*}
  \vec{v}_1=\colvec{x_1 \\ y_1}
  \quad
  \vec{v}_2=\colvec{x_2 \\ y_2}
\end{equation*}
from the plane~$P$, so that they satisfy the restriction
...
\begin{equation*}
  ...
  =\colvec{x_1+x_2 \\ y_1+y_2}
\end{equation*}
is then also a member of the plane $P$
...
\end{equation*}
and note that they are equal, because the entries are real numbers and 
real numbers commute. 
(That the vectors satisfy the restriction of lying on the line is not relevant;
...
zero element, and the vector of zeroes suffices.
\begin{equation*}
  \colvec{x \\ y}
  +\colvec{0 \\ 0}
  =\colvec{x \\ y}
\end{equation*}
Note that $\zero\in P$ as its second component is triple the first.
For~(5), that given any~$\vec{v}\in P$ we can
produce an additive inverse, just note that we have
\begin{equation*}
  \colvec{-x \\ -y}
  +\colvec{x \\ y}
  =\colvec{0 \\ 0}
\end{equation*}
so the vector $-\vec{v}$ is the desired additive inverse.
As with the prior condition, observe here that if $\vec{v}\in P$, so that
$y=3x$, then $-\vec{v}\in P$ also, since $-y=3(-x)$.

The checks for the five conditions having to do with scalar multiplication
are similar.
For~(6), closure under scalar multiplication,
suppose that $r\in\R$ and $\vec{v}\in P$
\begin{equation*}
  \vec{v}=\colvec{x \\ y}
\end{equation*}
so that it satisfies the restriction that~$y=3x$.
Then 
\begin{equation*}
...
the fact that its second component is three times
its first $ry=3(rx)$ follows from the restriction on~$\vec{x}$.
Next, this checks~(7).
\begin{equation*}
  (r+s)\cdot\colvec{x \\ y}
  =\colvec{(r+s)x \\ (r+s)y}
...
\begin{equation*}
  ...
  \colvec{rx \\ ry}
\end{equation*}
We shall check just two of the conditions.

For (1), closure of addition, observe that 
...
vectors that we must check.

The other closure condition, (6), is similar.
We have
\begin{equation*}
  r\cdot\colvec{x \\ y}
  =\colvec{rx \\ ry}
\end{equation*}
is a column array with two real entries, and so is a member of~\( \Re^2 \).
\end{example}
...
\nearbyexample{PlaneThruOriginSubsp} gives a subset of $\Re^2$ that is a
a vector space.
...
under the operations of component-wise 
