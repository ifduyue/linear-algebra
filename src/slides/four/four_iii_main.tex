...
\usepackage{colortbl}  % Just in this one file, to color Laplace tables 
...
\ExecuteMetaData[\detdir det3.tex]{dis:LaplaceIntro0}
\pause
\ExecuteMetaData[\detdir det3.tex]{dis:LaplaceIntro1}
\end{frame}

\begin{frame}
\ExecuteMetaData[\detdir det3.tex]{dis:LaplaceIntro2}
\pause
\ExecuteMetaData[\detdir det3.tex]{dis:LaplaceIntro3}
\end{frame}


\begin{frame}
\ExecuteMetaData[\detdir det3.tex]{dis:LaplaceIntro4}
\end{frame}
...
Earlier we found $S_{2,3}=7$ and the other two cofactors are here.
\begin{equation*}
  S_{2,1}=
  (-1)^{3}\cdot
  \begin{vmat}
...
is opposite to the what you might expect:~the entry
above in row~$i$ and column~$j$ is $T_{j,i}$.
\end{frame}
\begin{frame}
\ex  The matrix adjoint to 
\begin{equation*}
  S=
  \begin{mat}[r]
    3 &1 &2 \\
  ...
  \end{mat}
\end{equation*}
is this (we have already calculated some of these cofactors earlier).
\begin{equation*}
  \begin{mat}
