...
\iftoggle{showallproofs}{
  ...
  \begin{equation*}
    P=\set{\colvec{2 \\ 0 \\ 0},
           \colvec{0 \\ 2 \\ 0}}\subseteq\Re^3
  \end{equation*}
  If we expand that by adding a vector
  to get $\set{\vec{p}_1,\vec{p}_2,\vec{q}}$ 
  ...
  then the span is unchanged, as with $\spanof{P_0}=\spanof{P}$.
  But if the new vector is outside the starting span, $\vec{q}\notin\spanof{P}$,
  then the span grows, as in $\spanof{P_1}\supsetneq\spanof{P}$.
}
\end{frame}
