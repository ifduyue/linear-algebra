...
%<*dis:LaplaceIntro0>
In the permutation expansion of $\deter{T}$,
every term has one and only one entry from each row and column of~$T$.
\begin{equation*}
  \deter{T}=\!\!
...
%</dis:LaplaceIntro0>
%<*dis:LaplaceIntro1>
We can focus on any row or column; here we 
bring out the entries from $T$'s first row.
\begin{align*}
  &=
...
%</dis:LaplaceIntro1>
%<*dis:LaplaceIntro2>
Note that inside the first set of square brackets is the 
determinant
\begin{equation*}
  \begin{vmat}
    t_{2,2}  &t_{2,3} \\
...
%</dis:LaplaceIntro2>
%<*dis:LaplaceIntro3>
The explanation for these determinants lies with the permutations.
If we fix some row and column\Dash such as when we consider all terms
containing~$t_{1,1}$, or all terms containing~$t_{1,2}$, or~$t_{1,3}$\Dash then 
what remains is for the permutation to pick one entry from each of the other
rows and columns.

The matrices below illustrate.
On the left, the row and column containing~$t_{1,1}$ are shaded and 
what remains is the matrix from~($**$) above.
Similarly, in the other two what remains are the matrices from~($*\!*\!*$). 
...
%<*dis:LaplaceIntro4>
So that accounts for the appearance in equation~($*$) of the determinants
of smaller-sized matrices.
As to the minus sign on that equation's second line, 
...
\begin{equation*}
  \begin{aligned}[t]
    ...
    \end{aligned}
\end{equation*}
The matrices $P_{\phi_1}$ and~$P_{\phi_2}$
in the first line are all set up for the determinant~($**$),
since their first row is as in the identity matrix and their other two
rows look like the rows for that determinant.
The matrices $P_{\phi_3}$ and~$P_{\phi_4}$
in the second line will be set up in the same way with a single
row swap, putting~$\rowvec{1 &0 &0}$ into the first row.
Of course, that row swap puts a negative sign on that line.
Finally, setting up the third line's matrices
in the same way takes two
swaps, which keeps the sign positive.
%</dis:LaplaceIntro4>
...
\begin{example}
The $1,2$ cofactor of the matrix from equation~($*$)
is the negative of the first determinant in~($*\!*\!*$).
\begin{equation*}
  T_{1,2}=
  -1\cdot\begin{vmat}
    t_{2,1}  &t_{2,3}  \\
...
\begin{example}
A row or column with many zeroes makes a this expansion easier.
\begin{equation*}
  \begin{vmat}[r]
    1 &5  &0  \\
    2 &1  &1  \\
...
(or \definend{adjugate})\index{matrix!adjugate}\index{adjugate matrix}
to the square matrix \( T \) 
(or the \definend{classical adjoint})\index{classical adjoint} 
is
\begin{equation*}
  \adj(T)=
    \begin{mat}
...
\begin{proof}
The discussion before the theorem statement makes it clear.
\end{proof}
