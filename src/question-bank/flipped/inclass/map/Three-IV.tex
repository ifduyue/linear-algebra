...
Find $\rep{h}{B_W,B_X}$
\begin{solution}
This is the action of the map on $B_W$.
\begin{equation*}
  \colvec{1 \\ -1}\mapsunder{h}\colvec{2 \\ 0}
  \qquad
  \colvec{2 \\ 0}\mapsunder{h}\colvec{2 \\ 2}
\end{equation*}
Next, represent those with respect to the output basis.
\begin{align*}
  \colvec{2 \\ 0}=c_1\cdot \colvec{0 \\ 1}+c_2\colvec{2 \\ 2}
  &\quad\Longrightarrow\quad
  \rep{\colvec{2 \\ 0}}{B_X}=\colvec{-2 \\ 1}       \\
  \colvec{2 \\ 2}=c_1\cdot \colvec{0 \\ 1}+c_2\colvec{2 \\ 2}
  &\quad\Longrightarrow\quad
  \rep{\colvec{2 \\ 2}}{B_X}=\colvec{0 \\ 1}       
\end{align*}
So this is the matrix representing the map.
\begin{equation*}
  \rep{h}{B_W,B_X}=
...
\begin{equation*}
  ...
  \end{mat}
\end{equation*}
so the change in basis results in a change in the matrix representing the 
map.
\end{solution}
\end{parts}

\question
Consider the map $\map{\composed{h}{f}}{V}{X}$.
\begin{parts}
\part Define it.
\begin{solution}
We have this for the action of $\composed{h}{f}$.
\begin{equation*}
\colvec{a \\ b}
\mapsunder{f}
\colvec{a+b \\ 2b}
\mapsunder{h}
\colvec{(a+b)-(2b)  \\ (a+b)+(2b)}=\colvec{a-b  \\ a+3b}
\end{equation*}
\end{solution}
\part Represent it: find $\rep{\composed{h}{f}}{B_V,B_X}$.
\begin{solution}
This is the action of $\composed{h}{f}$ on the input basis.
\begin{equation*}
  \colvec{1 \\ 1}\mapsunder{h}\colvec{0 \\ 4}
  \qquad
  \colvec{1 \\ 0}\mapsunder{h}\colvec{1 \\ 1}
\end{equation*}
Next, represent those with respect to the output basis.
\begin{align*}
  \colvec{0 \\ 4}=c_1\cdot \colvec{0 \\ 1}+c_2\colvec{2 \\ 2}
  &\quad\Longrightarrow\quad
  \rep{\colvec{0 \\ 4}}{B_X}=\colvec{4 \\ 0}       \\
  \colvec{1 \\ 1}=c_1\cdot \colvec{0 \\ 1}+c_2\colvec{2 \\ 2}
  &\quad\Longrightarrow\quad
  \rep{\colvec{1 \\ 1}}{B_X}=\colvec{0 \\ 1/2}       
\end{align*}
So this is the matrix representing the map.
\begin{equation*}
  \rep{h}{B_W,B_X}=
  ...
  \end{mat}
\end{equation*}
Note that this is the same as this matrix-matrix product.
\begin{align*}
  \rep{B_V,B_X}{\composed{h}{f}}
  &=\rep{B_W,B_X}{h}\cdot\rep{B_V,B_W}{f}   \\
  &=H\cdot F                \\
  &=
  \begin{mat}
    -2  &0  \\
...
\part Apply it: find $\rep{\composed{h}{f}\,(\vec{v})}{B_X}$.
\begin{solution}
We have this.
\begin{equation*}
  \rep{\composed{h}{f}\,(\vec{v})}{B_X}
  =\rep{\composed{h}{f}}{B_V,B_X}\cdot\rep{\vec{v}}{B_V}
  =
  \begin{mat}
     4  &0  \\
     0  &1/2
