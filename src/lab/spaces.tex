...
You can ask \Sage{} questions about this subspace; for instance, you can 
test for membership. 
\begin{sagecommandline}
...
back is not the same as the set of two linearly
independent vectors $\set{\vec{v}_1, \vec{v}_2}$.
...
sage: W_prime.basis()
\end{sagecommandline}
If, on the other hand, you add to the spanning set a vector~$\vec{v}_3$ 
that is linearly dependent on the other two then
it doesn't change the subspace.
\begin{sagecommandline}
sage: W = V.span([v1, v2])       
sage: W.basis()
...
members of  $W_{1,2}$ then we have inclusion in the other direction and 
the two spaces are equal.
\begin{sagecommandline}
sage: V = VectorSpace(RDF,4)
sage: v1, v2 = vector(RDF, [1, 0, 0, 0]), vector(RDF, [1, 1, 0, 0])
sage: W12 = V.span([v1, v2])
sage: v3 = vector(RDF, [2, 1, 0, 0])
sage: W13 = V.span([v1, v3])  
sage: v2 in W13
...
using a real space that is just like the desired one.
