...
% \end{mat}
% \end{equation*}
by finding what it does to the four corners.
% \begin{equation*}
%   \colvec{0 \\ 0}\mapsto\colvec{0 \\ 0}
%   \quad
%   \colvec{1 \\ 0}\mapsto\colvec{a \\ b}
%   \quad
%   \colvec{1 \\ 1}\mapsto\colvec{a+c \\ b+d}
%   \quad
%   \colvec{0 \\ 1}\mapsto\colvec{c \\ d}
% \end{equation*}
Then we connect the four corners with line segments.
That is, linear transformations map squares to
parallelograms.

We start by ploting the unit square unchanged.
The colors will play the same role here as in the prior chapter;
more below.
\begin{sagecommandline}
...
\begin{equation*}
  ...
  \rowvec{ax+cy &bx+dy}
\end{equation*}
The source coude is at the end of the chapter.
