...
If you need a lot of newlines so the slash-n would be an annoyance then
you can put explicit line breaks inside a string marked by three double quotes.
\begin{pythonconsole}
...
\inlinecode{line}.
Thus, the final \inlinecode{if line:} above ejects any leftover material
...
For instance, for mathematical operations \python{} has a library, 
...
Instead, a text editor has the correct features for 
writing programs.
...
Here is a first example of a stand-alone \python{} program.
Start your editor, open a file called \path{month.py}, and enter these lines.
...
shows that \Sage{} adds a convenient shortcut for exponentiation over
