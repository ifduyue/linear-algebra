...
includes a variable \pythoncmd{apartment} then 
the right value for that variable when the person does not live in an
apartment is \inlinecode{None}.
...
\end{pythonconsole}
\noindent
A \python{} person who was not trying to illustrate 
...
you can use \pythoncmd{while}.\footnote{The 
...
\end{pythonconsole}
\noindent
Note that ``not equal'' is \inlinecode{!=}. 
The \inlinecode{//} operation is division between two integers, which
throws away the remainder.

Get out of a loop right away with the \inlinecode{break} command.
\begin{pythonconsole}
for i in range(10):
    if (i == 3):
        break
